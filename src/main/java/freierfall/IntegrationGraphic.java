...
class IntegrationGraphics extends JPanel {
	...
	boolean implEulerIntegration = false;
	...
	public void computeIntegratorValues(int integrationMethod) {
		...
		for (i = 1; i < 10000; i++) {
			...
			case 4:
				pos = new ImpliziterEuler().nextStep(pos, prevTime,
						timestepSize, ode);
				break;
	...
	public void updateIntegratorValues() {
		...
		if (implEulerIntegration)
			computeIntegratorValues(4);
	}
	...
	public void paintComponent(Graphics g) {
		...
		if (implEulerIntegration)
			drawIntegratorValues(Color.MAGENTA, 4, true);
	}
