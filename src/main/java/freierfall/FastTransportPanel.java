...
public class FastTransportPanel extends JPanel {
	 ...
	 * visualize / compute runge kutta steps
	 */
	JCheckBox checkBoxImplEuler;
	JTextArea textAreaImplEuler;
	...
	void updateGraphics() {
		...
		textAreaImplEuler.setText("Impliziter Euler X-zero-crossing: "
				+ String.valueOf((float) integrationGraphics.hitPointsX[4]));
	...
	public void initPanel() {
		 ...
		 * Impl Euler
		 */
		checkBoxImplEuler = new JCheckBox("Impliziter Euler (Magenta)", false);
		checkBoxImplEuler.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				integrationGraphics.implEulerIntegration = checkBoxImplEuler
						.isSelected();
				updateGraphics();
			}
		});
		c.gridy++;
		rightPanel.add(checkBoxImplEuler, c);

		textAreaImplEuler = new JTextArea();
		textAreaImplEuler.setEnabled(false);
		c.gridy++;
		rightPanel.add(textAreaImplEuler, c);
